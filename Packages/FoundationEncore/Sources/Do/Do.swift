...
@discardableResult
@inlinable
public func => <Subject, Pointee: AnyObject>(
    subject: Subject,
    assignment: (assignTo: ReferenceWritableKeyPath<Pointee, Subject>, on: Pointee)
) -> Subject {
    return subject => { assignment.on[keyPath: assignment.assignTo] = $0 }
}
